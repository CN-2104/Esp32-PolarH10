...
class MyClientCallback : public NimBLEClientCallbacks {
    ...
    void onConnect(NimBLEClient* pclient) {
        ...
        // These values are optimized for Polar H10 based on testing
        pclient->updateConnParams(120, 120, 0, 60);
        delay(200); // Short delay before requesting MTU
    }
...
bool connectToDevice() {
    ...
    // This helps resolve many connection issues
    NimBLEDevice::deinit(true);
    delay(500);
    NimBLEDevice::init("ESP32-HR-Monitor");
    ...
    // Use conservative connection parameters for better reliability with Polar
    // These values were determined through testing
     pClient->setConnectionParams(24, 48, 0, 700);
    pClient->setConnectTimeout(15);
    
    Serial.println("Trying connection with security disabled...");
    delay(800); // Longer delay before attempting - helps with Polar devices
    ...
    if (pClient->connect(polarH10Device)) {
        ...
        return true; // Return true even if service/char not found - we're still connected
    }
...
void setup() {
    ...
    NimBLEDevice::setMTU(185); // Larger MTU helps with stability and data transfer
...
void loop() {
    ...
    delay(10); // Small delay to prevent ESP32 watchdog triggering
}